jQuery(function($) {
    ...
    /**
     * On "Custom fields Image icon url" button click.
     */
    $('.cf_icon_modal').on('click', function(e) {
        // Keep category item click from being executed.
        e.stopPropagation();
        // Prevent navigating to '#'.
        e.preventDefault();

        $('#modal_cf_icon_url #displayData').html('');
        $('#modal_cf_icon_url').modal('show');
        $('#modal_cf_icon_url .loader').show();

        e.preventDefault();
        var $item = $(this).closest('li'),
            id = $item.data('quickad-field-id');

        var data = { action: 'CustomField_Icon_Url', id: id};

        $.post(ajaxurl+'?action=CustomField_Icon_Url', data, function(response) {
            if(response != "") {
                $('#modal_cf_icon_url #displayData').html(response);
                $('#modal_cf_icon_url .loader').hide();
            }else{
                quickadAlert({error: ['Problem in saving, Please try again.']});
            }
        });
    ...
    $('#ajax-send-custom-fields').on('click', function(e) {
        ...
        $.ajax({
            ...
            data      : { action: 'save_custom_fields', fields: JSON.stringify(data), cf_per_service: $cf_per_service.val() },
            complete  : function() {
                ladda.stop();
                quickadAlert({success : ["saved"]});
                //location.reload();
            }
        });
    });
    ...
    /**
     * Restore fields from custom_fields.
     */
