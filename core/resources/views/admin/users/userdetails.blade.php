...
<div class="quick-card card">
    <div class="card-body">
        ...
        <ul class="mb-0 list-unstyled">
            <li class="mb-2">
                {!! ___(':plan_ad_limit Listing Post Limits', ['plan_ad_limit' => '<strong>' . ($plan->settings->ad_limit == 999 ? ___('Unlimited') : number_format($plan->settings->ad_limit)) . '</strong>']) !!}
            </li>
            <li class="mb-2">
                {!! ___('Listing Expire in :plan_ad_duration Days', ['plan_ad_duration' => '<strong>' . ($plan->settings->ad_duration == 999 ? ___('Unlimited') : number_format($plan->settings->ad_duration)) . '</strong>']) !!}
            </li>
            <li class="mb-2">
        ...
        <button class="btn btn-primary w-100 mt-3 quick-open-slide-panel" type="button" data-panel-id="change-plan-panel"><i class="icon-feather-gift me-1"></i> {{___('Change Plan')}}</button>
    </div>
</div>
                    ...
                    <form id="slidepanel-inner-form" action="{{ route('admin.users.plan', $user->id) }}"
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" name="plan">
                                ...
                                <option value="trial" @if($plan->id == 'trial') selected @endif>{{___('Trial')}}</option>

                                @foreach($plans as $row)
                                    <option value="{{ $row->id }}" @if($plan->id == $row->id) selected @endif>{{$row->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-3">
                            ...
                            <select class="form-control" name="package_trial_done">
                                <option value="1" @if(user_options($user->id, 'package_trial_done')) selected @endif>{{___('Yes')}}</option>
                                <option value="0" @if(!user_options($user->id, 'package_trial_done')) selected @endif>{{___('No')}}</option>
                            </select>
                        </div>

                        <div class="mb-3 plan_expiration_date">
                            <label class="form-label" for="id_exdate">{{___('Expiration Date')}}</label>
                            <input id="id_exdate" type="date" class="form-control" name="plan_expiration_date" value="{{ $user->upgrade ? date_formating($user->upgrade->upgrade_expires, 'Y-m-d') : date('Y-m-d') }}">
                        </div>
                    </form>
                </div>
            </div>
