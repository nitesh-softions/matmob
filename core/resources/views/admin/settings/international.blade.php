        ...
        <div class="quick-card card">
            ...
            <div class="card-body">
                <div class="row g-3 mb-2">
                    <div class="col-lg-6">
                        <label class="form-label" for="country_type">{{ ___('Site Country Type') }} *</label>
                        <select name="country_type" id="country_type" class="form-select">
                            <option value="single" {{ @$settings->country_type == 'single' ? 'selected' : '' }}>
                                {{ ___('Single Country') }}</option>
                            <option value="multi" {{ @$settings->country_type == 'multi' ? 'selected' : '' }}>
                                {{ ___('Multi Countries') }}</option>
                        </select>
                    </div>
                    <div class="col-lg-6">
                        <label class="form-label" for="specific_country">{{ ___('Default Country') }} *</label>
                        <select class="form-control" name="specific_country">
                            <option value="" selected disabled>{{ ___('Choose') }}</option>
                            @foreach (countries() as $country)
                                <option value="{{ $country->code }}"
                                        @if ($country->code == @$settings->specific_country) selected @endif>
                                    {{ $country->name }}
                                </option>
                            @endforeach
                        </select>
